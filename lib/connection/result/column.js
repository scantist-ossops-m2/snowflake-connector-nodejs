...
function Column(options, index, statementParameters, resultVersion) {
  ...
  const metadataFields = options.fields;
...
function convertJsonObject(json, context) {
  context.metadataFields = context.metadataFields.reduce(function (map, obj) {
    map[obj.name] = obj;
    return map;
  }, {});
  const result = {};
  Object.keys(json).forEach(function (key) {
    result[key] = mapStructuredTypeFieldValue(json[key], context, key);
  });
  return result;
}
...
function convertJsonArray(json, context) {
  ...
  json.forEach(function (value) {
    result.push(mapStructuredTypeValue(value, context, context.metadataFields[0]));
  });
  return result;
}
...
function mapStructuredTypeValue(columnValue, context, metadataField) {
  ...
  case 'boolean':
    ...
    break;
  case 'timestamp_ltz':
    context.format = context.statementParameters['TIMESTAMP_LTZ_OUTPUT_FORMAT'] || context.statementParameters['TIMESTAMP_OUTPUT_FORMAT'];
    value = convertTimestampTzString(columnValue, context.format, context.statementParameters['TIMEZONE'], metadataField.scale).toSfDate();
    break;
  case 'timestamp_ntz':
    ...
    value = convertTimestampNtzString(columnValue, context.format, moment.tz.zone('UTC'), metadataField.scale).toSfDate();
    break;
  case 'timestamp_tz':
    context.format = context.statementParameters['TIMESTAMP_TZ_OUTPUT_FORMAT'] || context.statementParameters['TIMESTAMP_OUTPUT_FORMAT'];
    value = convertTimestampTzString(columnValue, context.format, context.statementParameters['TIMEZONE'], metadataField.scale).toSfDate();
    break;
  case 'date':
  ...
  case 'object':
    const internalContext = {
      convert: convertJsonObject,
      toValue: noop,
      toString: toString,
      format: toStringFromVariant,
      resultVersion: context.resultVersion,
      statementParameters: context.statementParameters,
      metadataFields: metadataField.fields
    };
    value = convertJsonObject(columnValue, internalContext);
    break;
  default:
    Logger.getInstance().info(`Column type not supported: ${context.metadataFields.type}`);
    throw new Error(`Column type not supported: ${context.metadataFields.type}`);
  }
  return value;
}

function mapStructuredTypeFieldValue(columnValue, context, fieldName) {
  const metadataField = context.metadataFields[fieldName];
  return mapStructuredTypeValue(columnValue, context, metadataField);
}
...
const convertDateTimeString = function (stringValue, formatSql, timezone, scale) {
  const formatMoment = dateTimeFormatConverter.convertSnowflakeFormatToMomentFormat(formatSql, scale);
  const momentValue = momentTimezone(stringValue, formatMoment);
  const epochSeconds = momentValue.unix();
  return new SfTimestamp(epochSeconds,momentValue.millisecond() * 1000000 , scale, timezone, formatSql);
};
